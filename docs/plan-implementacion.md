...
| Grupo/Condición/Status/Sub Status/Grade "para todo el ERP" = filtros y columnas en pantallas existentes (Productos) leyendo la última llegada, **sin** promoverlos a columnas propias de `Producto` | Agregar estos campos como columnas permanentes del producto (editables fuera del flujo de llegada) | El usuario pidió explícitamente "solo agregar filtros/columnas en pantallas existentes" al preguntársele por el alcance de "estos campos debemos usar para todo el ERP" |
